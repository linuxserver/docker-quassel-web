pipeline {
  ...
  environment {
    ...
    BUILD_VERSION_ARG = 'QUASSELWEB_COMMIT'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-quassel-web'
    CONTAINER_NAME = 'quassel-web'
    DOCKERHUB_IMAGE = 'linuxserver/quassel-web'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/quassel-web'
    PR_DOCKERHUB_IMAGE = 'lspipepr/quassel-web'
    DIST_IMAGE = 'alpine'
    MULTIARCH = 'true'
    CI = 'true'
    CI_WEB = 'true'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/quassel-web") | .uuid')
